...
if ($search) {
    $stmt = $conn->prepare("SELECT * FROM inventory WHERE item LIKE ? OR item_code LIKE ? ORDER BY item ASC");
    $likeSearch = "%$search%";
...
<form method="get" class="row mb-3">
    ...
    <div class="col-md-2"><a href="invent_list_page.php" class="btn btn-secondary w-100">Reset</a></div>
    <div class="col-md-2"><a href="add_inventory.php" class="btn btn-success w-100">Add Item</a></div>
</form>
...
<div class="table-responsive">
    <table class="table table-bordered table-striped mt-4">
        <thead class="table-dark">
            <tr><th>Item Name</th><th>Item Code</th><th>Unit Price (GHS)</th><th>In Stock</th><th>Min Level</th><th>Actions</th></tr>
        </thead>
        <tbody>
        <?php if ($items): ?>
...
document.querySelectorAll('.delete-btn').forEach(btn => {
    btn.addEventListener('click', function () {
        Swal.fire({
            ...
            if (result.isConfirmed) window.location.href = `delete_inventory.php?id=${this.dataset.id}`;
        });
    });
});
</script>
